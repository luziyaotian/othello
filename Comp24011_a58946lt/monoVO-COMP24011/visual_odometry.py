...
def featureMatching(image_ref, image_cur, matching_algorithm, threshold_value, output_path):
    ...
    #detects the keypoints in both images
    kp1 = detector.detect(image_ref, None)
    kp2 = detector.detect(image_cur, None)
   
    #the feature descriptors
    kp1,des1 = detector.compute(image_ref, kp1)
    kp2,des2 = detector.compute(image_cur, kp2)
    ...
    if (matching_algorithm==1):
        img3 = cv2.drawMatchesKnn(image_ref, kp1, image_cur, kp2, [good[:10]], None, flags=2)
        printMatchesToFile(distList, output_path)
    elif(matching_algorithm==2):
        img3 = cv2.drawMatchesKnn(image_ref,kp1, image_cur, kp2,matches,None,flags=2)
        printMatchesToFile(distList, output_path)
    elif(matching_algorithm==3):
        img3 = cv2.drawMatchesKnn(image_ref,kp1, image_cur, kp2,matches,None,**draw_params)
        printMatchesToFile(distList, output_path)
...
def printMatchesToFile(matches, output_path):
    ...
    for i in range(len(matches)):
        count = sum(1 for line in open(output_path))
        form = "{:.2f}".format(matches[i])
        output = (str(i) + ":"+" "+str(form)+"\n")
